...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!70, node distance=\layersep]
    ...
    \node[output neuron,pin={[pin edge={->}]right:}, right of=I-1] (O) at (3,-4.4) { $h_\theta (x)$};
   ...
   \node[annot,above of=I-1, node distance=1.7cm] (hl) {Entrada};
    \node[annot,right of=hl] at (3,-0.49) {Saída};
...
A Eq.\qref{fun:sigmoid} representa a função sigmoide, apresentada graficamente na Fig. \ref{fg:funcao_sigmoide}. 
